...
std::map<std::string, bencodeelement> decode_dict(std::vector<unsigned char> &encoded_data, size_t & position)
{
    ...
    while(encoded_data[position] != 'e')
    {
    std::string key = decode_str(encoded_data, position);
    if (encoded_data[position] == 'i')
    {
        decoded_element[key] = decode_int(encoded_data, position);
    }
    else if (encoded_data[position] == 'd')
    {
        decoded_element[key] = decode_dict(encoded_data, position);
    }
    else if(encoded_data[position] == 'l')
    {
        decoded_element[key] = decode_list(encoded_data, position);
    }
    else if(isdigit(encoded_data[position]))
    {
        if(key == "piece")
        {
            decoded_element[key] = decode_binary(encoded_data, position);
            
        }
        else
        {
            decoded_element[key] = decode_str(encoded_data, position);
        }
    }
    else
    {
...
std::vector<bencodeelement> decode_list(std::vector<unsigned char> encoded_data, size_t &position)
{
    ...
    while(encoded_data[position] != 'e')
    {
        if (encoded_data[position] == 'i')
    {
        decoded_data.push_back(decode_int(encoded_data, position));
    }
    else if (encoded_data[position] == 'd')
    {
        decoded_data.push_back(decode_dict(encoded_data, position));
    }
    else if(encoded_data[position] == 'l')
    {
        decoded_data.push_back(decode_list(encoded_data, position));
    }
    else if(isdigit(encoded_data[position]))
    {
            decoded_data.push_back(decode_str(encoded_data, position));
